...
for (t in 1:years){
  ...
  for (i in 2:NoSem){
      ...
      S0vh[i,j]<-S0vh[i-1,j]+S0vh[i-1,j]*G+S0pvh[i-1,j]*Woff-S0vh[i-1,j]*Lvh-S0vh[i-1,j]*(W+m[j,4])
      ...
      S0h[i,j]<-S0h[i-1,j]+S0h[i,j]*G+S0ph[i,j]*Woff-S0h[i-1,j]*Lh-S0h[i-1,j]*(W+m[j,4])
      ...
      S0l[i,j]<-S0l[i-1,j]+S0l[i,j]*G-S0l[i-1,j]*Ll-S0l[i-1,j]*(W+m[j,4])
      ...
      S0pvh[i,j]<-S0pvh[i-1,j]+S0vh[i,j]*Woff-S0pvh[i-1,j]*Lvh-S0pvh[i-1,j]*(W+m[j,4])
      ...
      S0ph[i,j]<-S0ph[i-1,j]+S0h[i,j]*Woff-S0vh[i-1,j]*Lh-S0ph[i-1,j]*(W+m[j,4])
...
P<-as.data.frame(cbind(1:NoSem,S0vh, S0h, S0l,S0vl,S0pvh,S0ph,I1vh, I1h, I1l, I1vl, I1pvh,I1ph,I2vh, I2h, 
                         ...
                         I8ph,AIDSvh, AIDSh, AIDSl, AIDSvl, AIDSpvh,AIDSph,Tr,Dead))
