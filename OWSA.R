OWSA<-function(Age=get("Age", globalenv()) , NoSem=get("NoSem", globalenv()),
  ...
  for (t in 1:years){
    for (j in 1:Age){
      ...
      for (i in 2:NoSem){
        ...
        S0vh[i,j]<-S0vh[i-1,j]+S0vh[i-1,j]*G+S0pvh[i-1,j]*Woff-S0vh[i-1,j]*(Lvh+W+m[j,4])
        ...
        S0h[i,j]<-S0h[i-1,j]+S0h[i-1,j]*G+S0ph[i-1,j]*Woff-S0h[i-1,j]*Lh-S0h[i-1,j]*(W+m[j,4])
        ...
        S0l[i,j]<-S0l[i-1,j]+S0l[i-1,j]*G-S0l[i-1,j]*Ll-S0l[i-1,j]*(W+m[j,4])
        ...
        S0vl[i,j]<-S0vl[i-1,j]+S0vl[i-1,j]*G-S0vl[i-1,j]*Lvl-S0vl[i-1,j]*(W+m[j,4])
        ...
        I1pvh[i,j]<-I1pvh[i-1,j]+S0pvh[i-1,j]*Lvh*RiskRed-I1pvh[i-1,j]*(m[j,5]+mVIH35+PTr[3])-I1pvh[i-1,j]*I12
        ...
        I1ph[i,j]<-I1ph[i-1,j]+S0ph[i-1,j]*Lh*RiskRed-I1ph[i-1,j]*(m[j,5]+mVIH35+PTr[3])-I1ph[i-1,j]*I12
        ...
        #New AIDS cases per year
        CalAIDS<-matrix(data=0, nrow=NoSem, ncol=Age)
        CalAIDS[i,j]<-sum(I8vh[i-1,j]*I89, I8h[i-1,j]*I89, I8l[i-1,j]*I89, I8vl[i-1,j]*I89, I8pvh[i,j], 
                          I8ph[i-1,j]*I89)
    ...
    P<-as.data.frame(cbind(1:NoSem,S0vh, S0h, S0l,S0vl,S0pvh,S0ph,I1vh, I1h, I1l, I1vl, I1pvh,I1ph,I2vh, I2h, 
                           ...
                           I8ph,AIDSvh, AIDSh, AIDSl, AIDSvl, AIDSpvh,AIDSph,Tr,Dead, Lvh_mat, Lh_mat, Ll_mat, Lvl_mat, CalAIDS, NewHIV))
    #
    Q[[t]]<-P
  }
