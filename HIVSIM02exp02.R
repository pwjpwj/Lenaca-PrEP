HIVSIM<-function(Age=get("Age", globalenv()) , NoSem=get("NoSem", globalenv()),
  ...
  # firs dimension weeks, second dimension age, third dimension S0---AIDS, forth dimention risk group and fifth prep
...
for (t in 1:years){
  ...
  for (i in 2:NoSem){
      ...
      PrevInfvh<-rowSums(Compartments[i-1,,2:10,1,])/rowSums(Compartments[i-1,,,1,])
      #Prevalence of infection in high risk group
      PrevInfh<-rowSums(Compartments[i-1,,2:10,2,])/rowSums(Compartments[i-1,,,2,])
      #Prevalence of infection in low risk group
      PrevInfl<-rowSums(Compartments[i-1,,2:10,3,])/rowSums(Compartments[i-1,,,3,])
      #Prevalence of infection in very low risk group
      PrevInfvl<-rowSums(Compartments[i-1,,2:10,4,])/rowSums(Compartments[i-1,,,4,])
