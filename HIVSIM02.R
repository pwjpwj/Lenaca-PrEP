HIVSIM<-function(Age=get("Age", globalenv()) , NoSem=get("NoSem", globalenv()),
  ...
  Q<-vector("list", years)
    ...
    AIDSvh[1,j]<-NumHSHVIH*nvh*ACD4*DistEdad[j]
...
for (t in 1:years){
  ...
  for (i in 2:NoSem){
      ...
      S0vl[i,j]<-S0vl[i-1,j]+S0vl[i,j]*G-S0vl[i-1,j]*Lvl-S0vl[i-1,j]*(W+m[j,4])
      ...
      S0pvh[i,j]<-S0pvh[i-1,j]+S0vh[i-1,j]*W-S0vl[i-1,j]*Lvh-S0pvh[i-1,j]*(Woff+m[j,4])
      ...
      S0ph[i,j]<-S0ph[i-1,j]+S0h[i-1,j]*W-S0vl[i-1,j]*Lh-S0ph[i-1,j]*(Woff+m[j,4])
      ...
      #New HIV treated
      NewTreatments[i,j]<-sum(I1vh[i-1,j], I2vh[i-1,j], I3vh[i-1,j],
                                            I4vh[i-1,j], I5vh[i-1,j], I6vh[i-1,j],
                                            I1h[i-1,j], I2h[i-1,j], I3h[i-1,j],
                                            I4h[i-1,j], I5h[i-1,j], I6h[i-1,j],
                                            I1l[i-1,j], I2l[i-1,j], I3l[i-1,j],
